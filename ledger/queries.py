...
def project_totals(conn, project_id=None):
    where = "WHERE f.project_id=?" if project_id is not None else ""
...
def project_list(conn, filters=None):
    ...
    FROM project p JOIN enterprise e ON p.enterprise_id=e.id LEFT JOIN funding f ON f.project_id=p.id"""
...
def project_detail(conn, project_id):
    ...
    result = dict(row)
    ent = conn.execute("SELECT * FROM enterprise WHERE id=?", (row["enterprise_id"],)).fetchone()
    result["enterprise"] = dict(ent) if ent else None
    result["fundings"] = [dict(x) for x in conn.execute("SELECT * FROM funding WHERE project_id=? ORDER BY id", (project_id,))]
    result["nodes"] = [dict(x) for x in conn.execute("SELECT * FROM node WHERE project_id=? ORDER BY plan_date,id", (project_id,))]
...
def dashboard(conn):
    ...
    return {**totals,
        ...
        "enterprise_count": conn.execute("SELECT COUNT(*) FROM enterprise").fetchone()[0]}
